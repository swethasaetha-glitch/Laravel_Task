...
class UpdateLayModelRequest extends FormRequest
{
    ...
    {
        $layModelId = $this->route('lay_model') ? ($this->route('lay_model')->id ?? $this->route('lay_model')) : null;

        return [
            'lay_model_code' => ['required', 'string', 'max:50', Rule::unique('lay_models', 'lay_model_code')->ignore($layModelId)],
